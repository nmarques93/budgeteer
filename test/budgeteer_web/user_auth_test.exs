defmodule BudgeteerWeb.UserAuthTest do
  ...
  describe "log_in_user/3" do
    test "stores the user token in the session", %{conn: conn, user: user} do
      ...
      assert Households.get_user_by_session_token(token)
    end
  ...
  describe "logout_user/1" do
    test "erases session and cookies", %{conn: conn, user: user} do
      user_token = Households.generate_user_session_token(user)
      ...
      assert redirected_to(conn) == ~p"/"
      refute Households.get_user_by_session_token(user_token)
    end
  ...
  describe "fetch_current_scope_for_user/2" do
    ...
    test "reissues a new token after a few days and refreshes cookie", %{conn: conn, user: user} do
      ...
      {user, _} = Households.get_user_by_session_token(token)
  ...
  describe "on_mount :require_sudo_mode" do
    ...
    test "redirects when authentication is too old", %{conn: conn, user: user} do
      ...
      user_token = Households.generate_user_session_token(user)
      {user, token_inserted_at} = Households.get_user_by_session_token(user_token)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        BudgeteerWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
