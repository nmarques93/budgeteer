defmodule Budgeteer.HouseholdsFixtures do
  @moduledoc """
  This module defines test helpers for creating
  entities via the `Budgeteer.Households` context.
  """
  ...
  def override_token_authenticated_at(token, authenticated_at) when is_binary(token) do
    Budgeteer.Repo.update_all(
      from(t in Households.UserToken,
        where: t.token == ^token
      ),
  ...
  def generate_user_magic_link_token(user) do
    {encoded_token, user_token} = Households.UserToken.build_email_token(user, "login")
    Budgeteer.Repo.insert!(user_token)
  ...
  def offset_user_token(token, amount_to_add, unit) do
    ...
    Budgeteer.Repo.update_all(
      from(ut in Households.UserToken, where: ut.token == ^token),
