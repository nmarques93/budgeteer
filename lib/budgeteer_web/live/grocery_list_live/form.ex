defmodule BudgeteerWeb.GroceryListLive.Form do
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope} online_members={@online_members}>
      <.header>
        {@page_title}
        <:subtitle>Use this form to manage grocery lists.</:subtitle>
      </.header>
      ...
      <.form for={@form} id="grocery-list-form" phx-change="validate" phx-submit="save">
        ...
        <footer>
          ...
          <.button navigate={return_path(@return_to, @grocery_list)}>Cancel</.button>
        </footer>
      </.form>
    </Layouts.app>
    """
  ...
  defp apply_action(socket, :edit, %{"id" => id}) do
    ...
    socket
    |> assign(:page_title, "Edit grocery list")
    |> assign(:grocery_list, grocery_list)
    |> assign(:form, to_form(Groceries.change_grocery_list(socket.assigns.current_scope, grocery_list)))
  end
  ...
  defp apply_action(socket, :new, _params) do
    ...
    socket
    |> assign(:page_title, "New grocery list")
    |> assign(:grocery_list, grocery_list)
    |> assign(:form, to_form(Groceries.change_grocery_list(socket.assigns.current_scope, grocery_list)))
  end
  ...
  def handle_event("validate", %{"grocery_list" => params}, socket) do
    changeset = Groceries.change_grocery_list(socket.assigns.current_scope, socket.assigns.grocery_list, params)
  ...
  defp save_grocery_list(socket, :edit, params) do
    case Groceries.update_grocery_list(socket.assigns.current_scope, socket.assigns.grocery_list, params) do
      {:ok, grocery_list} ->
        {:noreply,
         socket
